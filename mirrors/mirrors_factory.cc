...
map <string, mirrorFactory> registerMirrorFactories() {
    ...
    mirrorMethodMap["SQLITE"] = &sqlite_mirrors::createMirrors;
...
void printMirrors(map<string, mirror *> mirMap) {
    for (map<string, mirror *>::iterator it = mirMap.begin(); it != mirMap.end(); it++) {
        cout << "    - mirror: >" << it->first << "<: " << it->second->name << " " << it->second->desc << ", type: " << it->second->type << endl;
    }
}
...
map<string, mirror *> buildMirrors(map <string, mirrorFactory> mirrorFactoryMap, goptions go, runConditions rc) {
    ...
    // adding SQLITE
    mirrors *sqliteFactory = getMirrorFactory(&mirrorFactoryMap, "SQLITE");
    map < string, mirror * > sqliteMirs = sqliteFactory->initMirrors(rc, go);
    for (map<string, mirror *>::iterator it = sqliteMirs.begin(); it != sqliteMirs.end(); it++) {
        mirs[it->first] = it->second;
    }
