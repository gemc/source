...
// Dependent on 3 cartesian coordinates (Y, X, Z) expressed in cylindrical coordinate
// The values can be loaded from the map in any order as long as their speed is ordered.
// The values are indexed as B1_3D[Y][longi]
// The field is two dimensional, ordered in the class as B1=BT, B2=BL

// from fieldFactory:  load field map
void asciiField::loadFieldMap_general3d(gMappedField* map, double verbosity)
{
...
void gMappedField::GetFieldValue_general3d( const double x[3], double *Bfield, int FIRST_ONLY) const
{
	...
	if(symmetry == "general_3D-XYZ") {
		XX  = x[0];
		YY  = x[1];
		ZZ  = x[2];
	} else if(symmetry == "general_3D-XZY") {
		XX  = x[0];
		ZZ  = x[1];
		YY  = x[2];		
	} else if(symmetry == "general_3D-YXZ") {
		YY  = x[0];
		XX  = x[1];
		ZZ  = x[2];
	} else if(symmetry == "general_3D-YZX") {
		YY  = x[0];
		ZZ  = x[1];
		XX  = x[2];
	} else if(symmetry == "general_3D-ZXY") {
		ZZ  = x[0];
		XX  = x[1];
		YY  = x[2];
	} else if(symmetry == "general_3D-ZYX") {
		ZZ  = x[0];
		YY  = x[1];
		XX  = x[2];
	}
	...
	// outside map, returning no field
	if(IXX>=np[0]-1 || IYY>=np[1]-1 || IZZ>=np[2]-1) return;
	...
	if(interpolation == "none")
	{
		...
		if(symmetry == "general_3D-XYZ") {
			...
			Bfield[0] = B1_3D[IXX][IYY][IZZ];
			Bfield[2] = B2_3D[IXX][IYY][IZZ];
			Bfield[1] = B3_3D[IXX][IYY][IZZ];
		} else if(symmetry == "general_3D-YXZ") {
			Bfield[1] = B1_3D[IXX][IYY][IZZ];
			Bfield[0] = B2_3D[IXX][IYY][IZZ];
			Bfield[2] = B3_3D[IXX][IYY][IZZ];
		} else if(symmetry == "general_3D-YZX") {
			Bfield[1] = B1_3D[IXX][IYY][IZZ];
			Bfield[2] = B2_3D[IXX][IYY][IZZ];
			Bfield[0] = B3_3D[IXX][IYY][IZZ];
		} else if(symmetry == "general_3D-ZXY") {
			Bfield[2] = B1_3D[IXX][IYY][IZZ];
			Bfield[0] = B2_3D[IXX][IYY][IZZ];
			Bfield[1] = B3_3D[IXX][IYY][IZZ];
		} else if(symmetry == "general_3D-ZYX") {
			Bfield[2] = B1_3D[IXX][IYY][IZZ];
